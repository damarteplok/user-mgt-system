...
func GetUserById(db *sql.DB, id string) (models.User, error) {
	...
	err := db.
		QueryRow("SELECT id, email, password, name, category, dob, bio, avatar FROM users WHERE id = ?", id).
		Scan(&user.Id, &user.Email, &user.Password, &user.Name, &user.Category, &user.DOB, &user.Bio, &user.Avatar)
